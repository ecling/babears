 ...
 */
class Martin_Flytcloud_Model_Disifang_Orderonlinetools {
    ...
    public function __construct() {
        ...
        if(is_null(self::$soapClient) || !is_object(self::$soapClient)) {//多次操作有明显效果
            try {
                self::$soapClient = new SoapClient(Martin_Flytcloud_Model_Disifang_Config_Configuration::ORDERS_TOOLS_URLS,array(true));//这里的联网时间长达99.99%【应按需请求】
            } catch (Exception $e) {
                if(Martin_Flytcloud_Model_Disifang_Config_Struct::DEBUG) {
    ...
    private static function common($inputStructMethodName,$customerParameter) {
        try {
            $params = call_user_func_array(array('Martin_Flytcloud_Model_Disifang_Config_Struct',$inputStructMethodName),$customerParameter);

            $result = self::$soapClient->__soapCall($inputStructMethodName,array($params));
            $arr = $result->return->tracks->trackInfo;
            //$arr = Martin_Flytcloud_Model_Disifang_Config_Struct::outputStruct($result);
